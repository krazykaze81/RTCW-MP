...
void CG_AddFlameSpriteToScene( flameChunk_t *f, float lifeFrac, float alpha ) {
	...
	if ( sdist < f->size * 0.6 ) {
		...
		if ( Q_fabs( rdist[0] ) > radius || Q_fabs( rdist[1] ) > radius ) { // xMod modified
		...
		if ( !f->ignitionOnly &&
			...
			while ( fNext && !droppedTrail ) {
				if ( ( Distance( f->org, fNext->org ) < ( ( 0.1 + 0.9 * f->lifeFrac ) * f->size * 0.35 ) )
					 &&  ( Q_fabs( f->size - fNext->size ) < ( 40.0 ) ) // xMod modified
					 &&  ( Q_fabs( f->timeStart - fNext->timeStart ) < 100 ) // xMod modified
